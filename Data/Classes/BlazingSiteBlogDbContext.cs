using BlazingSite.Data.Interfaces;
using Microsoft.AspNetCore.Mvc.ApplicationModels;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using System;
using System.Collections.Generic;
using System.Diagnostics.CodeAnalysis;
using System.IO;
using System.Linq;
using System.Reflection;
using System.Threading;
using System.Threading.Tasks;

namespace BlazingSite.Data.Classes
{
    public class BlazingSiteBlogDbContext : DbContext
    {
        protected IConfiguration _config;

        public BlazingSiteBlogDbContext()
        {
            _config = new ConfigurationBuilder().AddJsonFile(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + @"\appsettings.json").Build();
        }

        public BlazingSiteBlogDbContext([NotNull] IConfiguration config)
        {
            _config = config;
        }

        protected override void OnModelCreating(ModelBuilder builder)
        {
            #region OnModelCreatingCode
            // Here we are Loading all types from assembly which have an interface of IBase, is not an abstract class, and is a public class.
            var types = Assembly.GetExecutingAssembly()
                .GetTypes()
                .Where(x => x.GetInterfaces().Any(_interface => _interface.Equals(typeof(IBase)) && x.IsClass && !x.IsAbstract && x.IsPublic));

            foreach (var type in types)
            {
                // OnModelCreating
                var onModelCreatingMethod = type.GetMethods().FirstOrDefault(x => x.Name == "OnModelCreating" && x.IsStatic);

                if (onModelCreatingMethod != null)
                    onModelCreatingMethod.Invoke(type, new object[] { builder });

                // OnBaseModelCreating
                if (type.BaseType == null || type.BaseType != typeof(Base))
                    continue;

                var baseOnModelCreatingMethod = type.BaseType.GetMethods().FirstOrDefault(x => x.Name == "OnModelCreating" && x.IsStatic);

                if (baseOnModelCreatingMethod == null)
                    continue;

                var baseOnModelCreatingGenericMethod = baseOnModelCreatingMethod.MakeGenericMethod(new Type[] { type });

                if (baseOnModelCreatingGenericMethod == null)
                    continue;

                baseOnModelCreatingGenericMethod.Invoke(typeof(Base), new object[] { builder });
                
            }
            #endregion
        }

        protected override void OnConfiguring(DbContextOptionsBuilder builder)
        {
            builder.UseSqlServer(_config["ConnectionStrings:BlazingSiteBlogDbContext"]);
        }

        public async override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
        {
            #region SaveChangesAsyncCode
            foreach (var entry in ChangeTracker.Entries())
            {
                if (entry.Entity is IBase)
                {
                    if (entry.State == EntityState.Added)
                        entry.Property("Created").CurrentValue = DateTimeOffset.Now;

                    else if (entry.State == EntityState.Modified)
                        entry.Property("LastUpdated").CurrentValue = DateTimeOffset.Now;
                }
            }

            return await base.SaveChangesAsync(cancellationToken);
            #endregion
        }
    }
}
